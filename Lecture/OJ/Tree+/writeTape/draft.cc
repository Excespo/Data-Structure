...
public:
    ...
    // MaxHeap(T* A, Rank n) {copyFrom(A,0,n); }
    ~MaxHeap() {};
...
int main() {
    ...
    // // test MaxHeap
    // int arr[] = {32,1,53,13,9,5,222,31,44,1,222};
    // MaxHeap<int> H1(arr, 11);
    // H1.show();
    // H1.descSort();
    // H1.show();
    // H1.remove(2);
    ...
    // // test MaxHeap deleteZeros
    // int arr2[] = {0,1,53,0,0,0,222,31,44,1,0};
    // MaxHeap<int> H2(arr2, 11);
    // H2.show();
    // H2.deleteZeros(5);
    // H2.show();
    ...
    // Use MaxHeap
    MaxHeap<int> H(a, n);
    printf("Case is %d files, %d heads with %d capa.\n", n, m, c);
    H.descSort(); cout<<"Sort: "; H.show();
    int time = 0, zeros = 0;
    while (!H.empty()) {
        cout <<"It's t = "<<time+1<<". We read/write ";
        m = (m > H.size()) ? H.size() : m; zeros = 0;
        // // Case where we can write a file discretely
        // for (int i = 0; i < m; ++i) {
        //     maxElem = H.delMax();
        //     cout << maxElem << ", ";
        //     if (maxElem > c) H.push_back(maxElem - c);
        //     (maxElem > c) ? cout << "(pushed "<<maxElem - c<<")|, " : cout << "(pushed x), ";
        // }
        for (int i = 0; i < m; ++i) {
            cout<<H[i]<<", ";
            (H[i] > c) ? cout << "(pushed "<<H[i] - c<<")|, " : cout << "(pushed x),";
            // H[i] = (H[i] > c) ? (H[i] - c) : 0;
            if (H[i] > c) H[i] = H[i] - c;
            else {H[i] = 0; zeros++;}
        }
